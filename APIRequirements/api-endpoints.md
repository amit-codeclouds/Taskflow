...
| GET | `/api/migrate/task/archived` | Auth | Archived tasks for a team (paginated). Query: `teamId`, `page`, `limit`, `statusId`, `search`. Powers the Board MFE **Archived Tasks** screen (`ArchivedTasklistComponent`, `TeamService.getArchivedTasks()`), the Board MFE's in-board **Archived Tasks panel** (`ArchivedTasksPanelComponent`, opened per-column from each archivable status's "View archived tasks" icon in `BoardComponent` — same service call, no `search` param used), and the Task MFE's **Archived** tab on `TeamTaskBoardScreen` (`/tasks/listview`, `archivedTasksService.list()`) |
...
| "View archived tasks" icon per archivable column header (BoardComponent → ArchivedTasksPanelComponent slide-over) | `GET /api/migrate/task/archived?teamId=&page=&limit=&statusId=&search=` via `TeamService.getArchivedTasks()` — same endpoint/service as the Archived Tasks table, rendered as a read-only in-context panel instead of a Kanban column (archived tasks aren't part of the live drag-and-drop flow). Shown on every `isArchievable` column, not just "Done" — opens with that column's status pre-selected in the panel's tabs (switchable to any other archivable status, or "All"). Row click navigates to `/archived-task/:taskId` |

...
import { LayoutDashboard, ListChecks, KanbanSquare, Users, ChevronRight, User, Settings, Sparkles, Sparkle } from 'lucide-react';
...
// task questions) — visually distinct from plain nav links, same treatment as
// the shell's own sidebar. Cross-zone (Shell owns /chat), so plain <a>.
const ASSISTANT_ITEM: NavItem = {
  label: 'Task Assistant', href: '/chat', icon: <Sparkles size={16} strokeWidth={1.5} />, external: true, highlight: true,
};
  ...
  return (
    ...
    >
      ...
      <div className="h-16 flex items-center px-5 shrink-0">
        {/* Home lives in the shell zone — plain <a> for cross-zone navigation, not <Link> */}
        <a href="/"><Logo /></a>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pt-3 pb-2">
        ...
        <nav className="flex flex-col gap-0.5">
          ...
          <NavLink {...ASSISTANT_ITEM} isActive={false} index={WORKSPACE_ITEMS.length + 1} />
        </nav>
        ...
        <div className="mt-5">
          ...
          <nav className="flex flex-col gap-0.5">
            {TOOLS_ITEMS.map((item, i) => (
              <NavLink key={item.label} {...item} isActive={false} index={WORKSPACE_ITEMS.length + 2 + i} />
            ))}
          </nav>
        </div>
      </div>

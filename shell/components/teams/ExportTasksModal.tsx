...
export default function ExportTasksModal({ team, onClose }: Props) {
  ...
  const formik = useFormik({
    initialValues: {
      fileName: `${slugify(team.name)}-tasks-export`,
    ...
    validationSchema: Yup.object({
      ...
      isIncludeArchiveTask: Yup.boolean().required(),
    }),
    onSubmit: async (values, { setSubmitting }) => {
      await exportMutation.mutateAsync({
        ...
        isIncludeArchiveTask: values.isIncludeArchiveTask,
      });
      setSubmitting(false);
      onClose();
    },
  ...
  const fileNameError = !!formik.touched.fileName && !!formik.errors.fileName;
  const extension = formik.values.format === 'Csv' ? '.csv' : '.xlsx';
          ...
          >
            <div className="px-6 py-5 flex flex-col gap-5">
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  {FORMAT_OPTIONS.map(({ value, label, icon: Icon }) => {
                    const selected = formik.values.format === value;
                    return (
                      <label
                        key={value}
                      ...
                      >
                        <input
                          ...
                          value={value}
                          checked={selected}
                          onChange={() => formik.setFieldValue('format', value)}
              ...
              <div>
                ...
                {fileNameError && (
                  <p className="text-xs text-status-red mt-1.5">{formik.errors.fileName}</p>
                )}
              </div>
              ...
              <div className="flex items-start gap-3">
                <input
                  ...
                  type="checkbox"
                  checked={formik.values.isIncludeArchiveTask}
                  onChange={e => formik.setFieldValue('isIncludeArchiveTask', e.target.checked)}
            ...
            <div className="px-6 pb-5 flex items-center justify-end gap-2.5">
              ...
              <motion.button
                type="submit"
                disabled={formik.isSubmitting || !!formik.errors.fileName}
